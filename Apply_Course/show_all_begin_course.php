		...
		if(count($_GET['query'])>0){
			...
			for($i=0; $i < count($_GET['query']); $i++){
				if($_GET['query'][$i]== 0){	//查出所有的課程
					$sql = "SELECT DISTINCT bc.begin_course_cd, bc.begin_course_name, bc.inner_course_cd, u.unit_name  FROM begin_course bc, lrtunit_basic_ u WHERE bc.begin_unit_cd=u.unit_cd and bc.apply_no!=0  ORDER BY bc.begin_course_cd ASC";
					//echo $sql;
				}
				else{
					...
					if($_GET['query'][$i] == 2) //開課單位
						...
						$patten .= "u.unit_name='".$_GET[unit_input]."' and ";
					if($_GET['query'][$i] == 3) //授課教師
						...
						$patten .= "p.personal_name='".$_GET[teacher_input]."' and ";
				}
			}
			if($patten != "")
			{
				$sql = "SELECT DISTINCT bc.begin_course_cd, bc.begin_course_name, bc.inner_course_cd, u.unit_name  FROM begin_course bc, lrtunit_basic_ u, teach_begin_course tc, personal_basic p WHERE ".$patten." bc.begin_unit_cd=u.unit_cd and tc.begin_course_cd=bc.begin_course_cd and tc.teacher_cd=p.personal_id ORDER BY bc.begin_course_cd ASC";
			}
