...
function get_school_list($location = -1,$type=0)
{
     $result = array(-1=>'不限');
     if(empty($location) or $location == -1)
         return $result;
     $sql = "SELECT school, school_cd
             FROM location
             WHERE city_cd = {$location}";
     if($type!=0)
         $sql .= " AND type = {$type}";

     $datas = db_getAll($sql);
     foreach($datas as $data)
         $result[$data["school_cd"]] = $data['school'];
...
function sql_takecourse_where($date_begin , $date_end , $pass=-1)
{
    ...
    if (isset($date_begin) && !empty($date_begin) ) {
        //$sql_where .= " AND take_course.pass_time >= '$date_begin 00:00:00' ";
        $sql_where .= " AND take_course.course_begin >= '$date_begin 00:00:00' ";
    }
    if (isset($date_end) && !empty($date_end)) {
        //$sql_where .= " AND take_course.pass_time <= '$date_end 23:23:59' ";
        $sql_where .= " AND take_course.course_end <= '$date_end 23:23:59' ";
//        $sql_where .= " AND take_course.send2nknu_time  <= '$date_end 23:23:59' ";
    }*/

    if (isset($date_begin) && !empty($date_begin) 
        && isset($date_end) && !empty($date_end)
        ) 
    {
        $sql_where .= "AND ( (take_course.course_begin BETWEEN '$date_begin' AND '$date_end') OR (take_course.course_begin < '$date_begin' AND take_course.course_end > '$date_end'))";
