...
if( $flag_download_excel != true ) {
	assignTemplate($tpl, '/apply_course/stat_request1.tpl');
} else {
	$search_cond = gen_search_cond_value($input_check) ; 

    $course_data_row = retrive_request1_data(array(
...
function retrive_request1_data($config)
{
    //get param
    $course_property = (isset($config['course_property']))?$config['course_property']:-1;
    $begin_course_cd = (isset($config['begin_course_cd']))?$config['begin_course_cd']:-1;
    $start           = (isset($config['start']))?$config['start']:-1;
    $end             = (isset($config['end']))?$config['end']:-1;
    $city_cd         = (isset($config['city_cd']))?$config['city_cd']:-1;
    $doc_cd          = (isset($config['doc_cd']))?$config['doc_cd']:-1;
    

    //---------------------------------------------
    // get doc info
    //---------------------------------------------
    $sql = "SELECT doc_cd,doc FROM docs WHERE 1 ";
    if($city_cd != -1)
        $sql .= "AND city_cd = $city_cd ";
    if($doc_cd != -1)
    ...
    {
        $doc_list[$dataRow['doc_cd']]=$dataRow['doc'];
    }
    
    //--------------------------------------------- 
    //get course_properyties
    //--------------------------------------------- 
    
    $course_properties = get_course_property();

    //---------------------------------------------
    // get course info
    //---------------------------------------------
    $sql_date_start_where = ($start != -1) ? " AND course_begin >= $start" : "";
    $sql_date_end_where = ($end != -1) ? "AND course_end <= $end" : "";
    $sql_begin_course_cd  = ($begin_course_cd != -1)? "AND begin_course_cd=$begin_course_cd":"";
    
    $sql_take_course = "(SELECT begin_course_cd, personal_id, pass,course_begin,course_end 
                         FROM take_course 
                         WHERE 1 $sql_begin_course_cd $sql_date_start_where $sql_date_end_where)";

    $sql_personal_basic = "(SELECT personal_id, doc_cd, sex, d_birthday, job, familysite
                            FROM personal_basic
                            WHERE dist_cd=0 $personal_cond_where)";
    
    $sql_begin_course = ($course_property != -1)?
                        "(SELECT begin_course_cd, begin_course_name,course_property,certify FROM begin_course WHERE course_property = $course_property)" : 
                        " begin_course";
    
    
    $sql = "SELECT DISTINCT * 
            FROM $sql_take_course t, $sql_begin_course b, $sql_personal_basic p
            WHERE t.begin_course_cd = b.begin_course_cd 
    ...
    while($dataRow = $result->fetchRow(DB_FETCHMODE_ASSOC))
    {
        //年 月 doc 課程性質 課程名稱 統計
        if(empty($doc_list[$dataRow['doc_cd']]))
            continue;
        $key = $dataRow['doc_cd'].$dataRow['begin_course_cd'];
        
        $birthday = strtotime($dataRow['d_birthday']);
        $data[$key]['doc'] = $doc_list[$dataRow['doc_cd']];
        $data[$key]['begin_course_name'] = $dataRow['begin_course_name'];
        $data[$key]['course_property'] = $course_properties[$datRow['course_property']];
        $data[$key]['total_count'] += 1;

        if($dataRow['sex']==1){
            $data[$key]['data1'] += 1;

            if($birthday >= (time - 1576800000))
                $data[$key]['data6'] += 1;
        
            if($dataRow['job']==0)
                $data[$key]['data8'] += 1;
            
            if($dataRow['familysite']==3)
                $data[$key]['data3'] += 1;
        }
        else{
        ...
        $data[$key]['total_hour'] += $dataRow['certify'];
    }
