...
if ($has_data) {
    ...
    if($order_method == 3) {
        $sql_sub_begin_course_cd = " SELECT begin_course_cd, begin_course_name, certify FROM begin_course where begin_course_cd in (42,54,55,56,57,58,59,60,62)" ;
        $all_course = db_getAll( $sql_sub_begin_course_cd );
        $result = list_course_group_by_location($all_course, get_location_list(0),$flag_download_excel);
    }
...
if( $flag_download_excel != true ) {
    assignTemplate($tpl, '/apply_course/stat_request2.tpl');
} else {
...
function list_course($all_course,$to_excel=false) {
    ...
    foreach( $all_course as $course) {
        ...
        $sql_take_course_stus = " select personal_basic.personal_id, personal_name,personal_basic.city_cd, personal_basic.school_type, personal_basic.school_cd, pass,take_course.pass_time, take_course.send2nknu_time FROM take_course "
            ." LEFT JOIN personal_basic ON take_course.personal_id=personal_basic.personal_id "
            ." WHERE begin_course_cd={$course['begin_course_cd']} $sql_takecourse_where  $sql_where_location " ;
        ...
        $has_date_condition = !empty($date_begin) && !empty($date_end);
        
        if($has_date_condition)
        {
            $start_timestamp = strtotime($date_begin);
            $end_timestamp = strtotime($date_end);
        }
        foreach($takecourse_stus as $stu) {
            $course_total_stu ++ ;
                $pass_time = $stu['pass_time']!='0000-00-00'? strtotime($stu['pass_time']):0;
            if( !$has_date_condition && $stu['pass'] == 1 
                or $has_date_condition && $start_timestamp <= $pass_time && $pass_time <= $end_timestamp) { //通過的學生
                ...
                $passed_stus_with_time[] = array($stu['personal_name'],$stu['city_cd'],$stu['pass_time'],$admin) ; 	
            }else { //尚未通過的學生
...
function list_course_group_by_location($all_course , $location,$to_excel=false) {
    ...
    foreach( $location as $key => $value) {
        ...
        foreach( $all_course as $course) {
            ...
            $sql_take_course_stus = " select personal_basic.personal_id, personal_name, pass, personal_basic.city_cd, personal_basic.school_cd , take_course.send2nknu_time , take_course.pass_time FROM take_course "
                ." LEFT JOIN personal_basic ON take_course.personal_id=personal_basic.personal_id "
                ." WHERE begin_course_cd={$course['begin_course_cd']} $sql_takecourse_where  $sql_where_location " ;
            ...
            foreach($takecourse_stus as $stu) {
                ...
                $pass_time = $stu['pass_time']!='0000-00-00'? strtotime($stu['pass_time']):0;
                if( !$has_date_condition && $stu['pass'] == 1 
                     or $has_date_condition&& $stu['pass']==1 && $start_timestamp <= $pass_time && $pass_time <= $end_timestamp) { //通過的學生
                    $course_total_pass_stu ++ ;
