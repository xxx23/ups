...
//???o?Ч??W?? ?P ?Юv?W?? 
$sql_get_content_name = " SELECT content_name FROM class_content_current A, course_content B WHERE  A.content_cd = B.content_cd AND A.begin_course_cd = $begin_course_cd ";
$content_name = db_getOne($sql_get_content_name) ; 
$tpl->assign("content_name", $content_name);

$sql_get_teacher_name = " SELECT B.personal_name FROM teach_begin_course A, personal_basic B WHERE A.begin_course_cd=$begin_course_cd AND A.teacher_cd=B.personal_id AND A.course_master=1";
$teacher_naem = db_getOne($sql_get_teacher_name) ; 
$tpl->assign("teacher_name", $teacher_naem);
...
//???X????
assignTemplate($tpl, "/apply_course/view_course_detail.tpl");
