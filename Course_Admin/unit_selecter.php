<?php
	...
	if(!empty($unit_tree)){
		foreach($unit_tree['-1'] as $treenode)
		{
			if( count($unit_tree["".$treenode['unit_cd']])>0){
				$contentStr .= '<li class="closed"><span class="folder" onClick ="transferToOpener(\'begin_unit_name\',\''.$treenode['unit_name'].'\',\''.$treenode['unit_cd'].'\')">'.$treenode['unit_name'].'</span>';
				$contentStr .= traversal($treenode,$unit_tree);
				$contentStr .= '</li>';
			}else{
				$contentStr.= '<li><span class="file" onClick ="transferToOpener(\'begin_unit_name\',\''.$treenode['unit_name'].'\',\''.$treenode['unit_cd'].'\')">'.$treenode['unit_name'].'</span></li>';
			}
		}
	}
	$contentStr .= '</ul>';
	...
	function traversal($node,$unit_table)
	{
		...
		foreach($unit_table["".$node['unit_cd']] as $treenode)
		{
			...
			{
				$tempStr.='<li class="closed"><span class="folder" onClick ="transferToOpener(\'begin_unit_name\',\''.$treenode['unit_name'].'\',\''.$treenode['unit_cd'].'\')">'.$treenode['unit_name'].'</span>';
				$tempStr.= traversal($treenode,$unit_table);
				$tempStr.= '</li>';
			}
			else
				$tempStr .='<li><span class="file" onClick ="transferToOpener(\'begin_unit_name\',\''.$treenode['unit_name'].'\',\''.$treenode['unit_cd'].'\')">'.$treenode['unit_name'].'</span></li>';
		}
		$tempStr .= '</ul>';
