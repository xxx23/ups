...
class ValidateBeginCourse
{
	...
	public function ValidatePHP($attribute)
    {
        ...
        if($attribute == 1) //教導式才需要validate時間
        {
            if(!$this->ValidateSelectDate($_POST))                
            {
                    $_SESSION['errors']['begin_course_name'] = 'error';                    
                    $errorsExist = 1;
            }
            
            if(!$this->ValidateCourseDate($_POST))
            {
                    $_SESSION['errors']['begin_course_name'] = 'error';
                    $errorsExist = 1;
            }
        }
        // 如果是自學式的話，因為要調整選課時間、課程開始時間和結束時間 所以就要另外assign
        ...
        if($attribute == 0)
        {
            $d_course_begin = $today;
			$d_course_end = $class_end;
			$d_public_day = $today;
		...
		if ( $errorsExist == 0){
			...
			if (PEAR::isError($res))	die($res->getMessage());
			//清除剛剛塞入的validateKey
			$sql = "UPDATE begin_course SET note=NULL , begin_coursestate='0', course_type='0' WHERE  begin_course_cd='$begin_course_cd'";
			$res = $this->pearDB->query($sql);
			if (PEAR::isError($res))	die($res->getMessage());
...
//echo $streamingDir ;
	...
	private function ValidateCourse_Property($value)
	{
		...
		if($value != -1){
			...
			$this->setErrorMsg("課程性質尚未選擇11111");
			return 0; //not valid	
		}	
	}
    ...
    private function ValidateCourse_Stage_update_session($value1, $value2, $value3, $value4)
    {
        ...
        {
            $this->setErrorMsg("課程階段屬性欄位不可為空白111111");
            return 0;
        }
    }
    ...
    private function ValidateCriteria_Total($value)
    {
        ...
        elseif( $value == 0) // 如果等於0分的話也可以輸入！
        {
            return 1;
        }
        else
        {
            $this->setErrorMsg("課程總分尚未填入或是格式錯誤。");
            return 0 ;
        }
    }
