...
//AJAX驗證欄位資料
	...
	public function ValidatePHP()
	{
		...
		if( !$this->validateName($_POST['txtName']))
		{
            ...
            $_POST['txtName']=clean_param($_POST['txtName'], PARAM_CLEAN);
        }
		...
		if ( $errorsExist == 0){
			...
			//inset into DB
            if( !empty($_SESSION['register']['login_id']) && !empty($_SESSION['register']['pass']) ){
                $personal_id = create_account($_SESSION['register']['login_id'],$_SESSION['register']['pass'],$_SESSION['role_type']);
                if($_SESSION['role_type'] == 1 ) {// 教師的話需要新增ftp帳號 
                    require_once('../library/ftp_func.php') ;
                    global $DATA_FILE_PATH ; 
                    if( !is_dir($DATA_FILE_PATH.$personal_id.'/') ){
                        createPath($DATA_FILE_PATH.$personal_id.'/');
                    }
                    new_ftp_account( $_SESSION['register']['login_id'], $_SESSION['register']['pass'] , '');
                }
            }else
            {
                die("註冊發生錯誤請重新操作 <br/> <a href=\"https://ups.moe.edu.tw\">回首頁</a>");
            }
			...
			//echo $interest;
            //echo $sql;
            $_POST[txtZoneCd] = clean_param($_POST[txtZoneCd],PARAM_CLEAN);
            $_POST[txtAddr] = clean_param($_POST[txtAddr],PARAM_CLEAN);
            $_POST[txtID] = strtoupper($_POST[txtID]);
            $_POST[txtID] = trim($_POST[txtID], "　 ");
            if( $personal_id != ''){
				$sql = "
	...
	private function validateID($value)
	{
		...
		$sql = "SELECT *
			...
			WHERE pb.personal_id = rb.personal_id AND pb.identify_id = '{$value}'AND rb.role_cd = {$_SESSION['role_type']}";
		$res = $DB_CONN->query($sql);
		if(PEAR::isError($res)) die($res->getMessage());
		$res->fetchInto($row, DB_FETCHMODE_ASSOC);

        $upper_case = isset($row['personal_id'])?true:false;
        ...
        $lower_case = isset($row['personal_id'])?true:false;
