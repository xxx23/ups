...
function learning_status($Content_cd, $Menu_id, $Personal_id)
{
	...
	else if($USE_MONGODB)
	{
		...
		$result = $student_learning->findOne(array('bcd' => intval($Begin_course_cd), 'ccd' => intval($Content_cd), 'pid' => intval($Personal_id), 'mid' => intval($Menu_id)));

		if($result == NULL)
		{
			$Event_occur_time = TIME_date(2) . " " . TIME_time(2);
			$student_learning->insert(array('bcd' => intval($Begin_course_cd), 'ccd' => intval($Content_cd), 'pid' => intval($Personal_id), 'mid' => intval($Menu_id), 'ehn' => 1, 'eht' => 0, 'eot' => new MongoDate(strtotime($Event_occur_time)), 'elt' => new MongoDate(strtotime($Event_occur_time))));
		}
		else
		{
			$row = $result;
			$count = $row['ehn'] + 1;
			$Event_last_time = TIME_date(2) . " " . TIME_time(2);
			$student_learning->update(array('bcd' => intval($Begin_course_cd), 'ccd' => intval($Content_cd), 'pid' => intval($Personal_id), 'mid' => intval($Menu_id)), array('$set' => array('elt' => new MongoDate(strtotime($Event_last_time)), 'ehn' => intval($count))));
		}
	}
...
function record_hold_time($Menu_id,$Personal_id,$Begin_course_cd)
{
	...
	else if($USE_MONGODB)
	{
		...
		$student_learning->update(array('bcd' => intval($Begin_course_cd), 'pid' => intval($Personal_id), 'mid' => intval($Menu_id)), array('$set' => array('eht' => intval($Event_hold_time) + $t)));
	}
