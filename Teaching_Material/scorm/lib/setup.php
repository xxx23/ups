...
/// Time to start counting    
    init_performance_info();//lib/setuplib.php        
...
/// and configure some other specific variables for each db BEFORE attempting the connection
     preconfigure_dbconnection();//lib/setuplib.php

    require_once($CFG->libdir .'/adodb/adodb.inc.php'); // Database access functions

    $db = &ADONewConnection($CFG->dbtype);//lib/adodb/adodb.inc.php
...
/// Increase memory limits if possible
    raise_memory_limit('96M');    // We should never NEED this much but just in case... ///lib/setuplib.php
...
/// and configure some other specific variables for each db
    configure_dbconnection();//lib/dmllib.php

/// Load up any configuration from the config table
    $CFG = get_config();//lib/moodlelib.php
...
/// Load up global environment variables
    ...
    if (defined('FULLME')) {     // Usually in command-line scripts like admin/cron.php
        ...
        $FULLME = qualified_me();
        $ME = strip_querystring($FULLME); //lib/weblib.php
    }
...
/// majority of cases), use the stored locale specified by admin.
    ...
    setup_lang_from_browser();//lib/moodlelib.php
    ...
    course_setup();//lib/moodlelib.php
