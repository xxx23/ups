<?PHP  // $Id: player.php,v 1.34.2.9 2008/11/14 00:14:50 piers Exp $
    ...
    //
   // $bodyscript = 'onbeforeunload=recordtime()';
   $bodyscript = 'onbeforeunload=recordtime('.$begin_course_cd.','.$personal_id.')';
    //$bodyscript = '';
    if ($scorm->popup == 1) {
    ...
    print_header("瀏覽教材開啟，請注意：為避免閱讀時數無法記錄，請於登出前將閱讀子視窗關閉!".$pagetitle, $course->fullname,
                 $navigation,
...
.splitter-bar-vertical-docked {
    width: 6px;
    background-image: url(vdockbar-trans.gif);
    background-repeat: no-repeat;
    background-position: center;
}
...
/*function recordtime(){
     ...
     xmlhttp=GetXmlHttpObject();
      ...
      var url="getclosetime.php";
      xmlhttp.open("GET",url,true);
      xmlhttp.send(null)
}*/
function recordtime(begin_course_cd,personal_id){
    xmlhttp=GetXmlHttpObject();
      if (xmlhttp==null)
            {
                alert ("Browser does not support HTTP Request");
      ...
      var url="getclosetime.php?Begin_course_cd="+begin_course_cd+"&Personal_id="+personal_id;
      xmlhttp.open("GET",url,true);
