...
else//非scorm.zip的教材處理
{
	...
	//顯示scorm 2004匯出狀況
		...
		assignTemplate( $smtpl,"/teaching_material/textbook_export.tpl");
}//非scorm.zip的教材處理
...
function show_status($prefix, $file_name, $teacher_cd)
{
	...
	if(file_exists($file_path)){
        ...
        if($lang=="zh_tw"){
            ...
            $smtpl->assign($prefix."export_exist",0);
            $smtpl->assign($prefix."export_file_size","export yet");
            $smtpl->assign($prefix."export_file_time","export yet");
            $smtpl->assign($prefix."export_file_path","export yet");
        }
	}
}
