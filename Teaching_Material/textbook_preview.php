<?php
/***************************************************/
/* id:textbook_preview v1.0 2007/4/26 by hushpuppy */
/* function: 預覽本課程教材						   */
/***************************************************/
include "../config.php";
...
if($Content_cd != 0){
  ...
  $scorm_id= $DB_CONN->getOne($sql);
}
$Role_cd = $_SESSION['role_cd'];
//print "personal_id:".$_SESSION['personal_id'];
if(!isset($scorm_id))
{
	if(!isset($Content_cd) || $Content_cd == 0){
		if($Role_cd == 1){
			...
			echo "課程教師尚未指定本課程使用教材，目前尚無教材。";
	}
	else
		header("Location: ./stu_loadTreeFromDB.php?content_cd=$Content_cd");
}
else
{
  // echo "按<font color=\"red\"><a href=\"./scorm/mod/scorm/view.php\" target=_blank>這裡</a> </font>另開新視窗瀏覽scorm教材";
   header("Location: ./scorm/mod/scorm/OpNewWin.php");
}
?>
