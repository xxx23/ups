...
{literal}
	...
	{
		var theFrame = $('#transfer_log', window.parent.document.body);
...
<div class="searchbar" style="width:80%;margin:auto auto;"> 
    <form id="timeSearch" name="timeSearch" action="nknu_transfer_log.php" method="POST">
	...
	{foreach from=$logs item=log}
		<tr class="control">
		<td>{$log.log_date|escape}</td>
		<td>{$log.Import_ID|escape}</td>
		<td>{$log.log_level|escape}</td>
		</tr>
	...
	{/foreach}
    {else}
        <td colspan="3"><center>無資料</center></td>
    {/if}
	</table>
</div>
</body>
