...
<tr>
	<th width="300">課程屬性</th>
	<td>
	{if $attribute eq 1} 教導式 {/if} 
	{if $attribute eq 0} 自學式 {/if} 
	</td>
</tr>
<
<tr>
...
<tr>
	...
	<td>    {if $deliver eq 1}
		...
		{else}
            本課程不需傳送認證時數
            <a href="preview_content.php?begin_course_cd={$begin_course_cd}" target="_blank">預覽教材</a>
...
<tr>
        <th>評量標準(總分)</th>
      <td>{if $criteria_total eq 0}不須測驗{else} {$criteria_total}{/if} </td>
</tr>
{if $attribute == 1}
<tr>
        <th>評量標準(線上成績)</th>
...
{/if}
			...
			<form action="verify_course.php"  method="post">
            <table class="datatable"><tr><th>
				<input name="begin_course_cd" type="hidden" value="{$begin_course_cd}"/>
				<input name="pass" type="hidden" value="true"/>
				<input type="submit" value="通過" />
				</th><td>通過的帳號將可以進行開課。</td></tr>
			</table>
            </form>
	<div>
		<form action="verify_course.php"  method="post">
		...
		<tr><th><input  type="submit" value="不通過" method="post"></th>
        <td>原因：<br/>
			<textarea id="state_note" name="state_note" cols="45" rows="5"></textarea>
            </td></tr>
			</table></form>
	</div>
</div>
