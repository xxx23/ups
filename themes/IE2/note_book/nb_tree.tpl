...
//		tree.treeOnNodeChange = function(node){changeNode(node)};
		...
		tree.opt.sort = "desc" ;
		{/literal}
		ctx.add("1", "新增一個筆記本目錄", "", "{$webroot}script/nlstree/img/newnode.gif");
		ctx.add("2", "刪除此筆記本目錄", "", "{$webroot}script/nlstree/img/nodrop.gif");
		ctx.add("3", "編輯此筆記本目錄名稱", "", "{$webroot}script/nlstree/img/config.gif");
		ctx.addSeparator();
		...
		ctx2.add("1", "新增一個筆記本目錄", "", "{$webroot}script/nlstree/img/newnode.gif");
...
<div id="contents" width="300" height="300">
	<div id="nav" style="float:left;width:20%;height:100%;">
	    ...
	    <!--<div class="area" id="message" style="overflow-x:hidden;" onMouseOver="document.getElementById('message').style.overflowX='visible';" onMouseOut="document.getElementById('message').style.overflowX='hidden';"><script>tree.render();</script>
		</div>-->
		<div>
