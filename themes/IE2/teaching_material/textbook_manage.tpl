...
$(document).ready(function(){
		$("#set_downloadRole").hide();
		$("#set_license").hide();
        $("#modify_isDownload_show").hide();
        $("#create_isDownload").change(function(){
        if($("#create_isDownload").val()==1)
        {
			$("#set_downloadRole").show();
			$("#set_license").show();
        }
        else
        {
...
function modify_isDownload_func(option)//joyce add 0330
{
    if(option==1)
    {
		document.getElementById("modify_downloadRole").style.display="";
		document.getElementById("modify_license").style.display="";
    }     
    else
    {
		document.getElementById("modify_downloadRole").style.display="none";
        document.getElementById("modify_license").style.display="none";
    }     
}
// ************ ************ ************ ************ ************ ************ 
...
function display(option){
	...
	if(option == 1){
		...
		document.getElementById("inner_contentI").style.display = "";
		document.getElementsByTagName("body")[0].id = "tabI";
	}
}
...
function callBack(){
//alert('callBack'+xmlHttp.readyState);
...
{	
	if(document.getElementById("create_isDownload").value == 1)
	{
...
{	
	if(document.getElementById("modify_isDownload").value == 1)
	{
...
<div class="tab">
  <ul id="tabnav">
    ...
    <!-- joyce 0501  -->
    {if $begin_course_cd neq "" }
        <li class="tabI" onClick="display(9)">匯入SCORM教材至此</li>
    {/if}
  </ul>
</div>
<div id="inner_contentA">
{if $begin_course_cd neq "" }
    <p class="intro">本課程目前使用的教材：<span class="imp">{$TEXTBOOK}</span>
    &nbsp;&nbsp;&nbsp;此教材的作者為：<span class="imp">{$textbook_master}</span>
    &nbsp;&nbsp;&nbsp;
    <a href="textbook_manage.php?choose_textbook_this=0">(移除指定)</a>
{/if}
</p>
...
<tr>
	...
	<th style="text-align:center">教材名稱&nbsp;&nbsp;(指定教材)</th>	
...
<tr class="{cycle values=",tr2"}">
    ...
    {if $begin_course_cd neq "" }
        &nbsp;&nbsp;<a href="textbook_manage.php?choose_textbook_this={$k}">(指定此教材)</a></td>
    {/if}    
...
<iframe name="show_export" id="show_export" onload="ResizeIframe(this,350);" src="./export_textbook.php?" style="display:none; "> </iframe>
<br/>
...
<div class="inner_contentB" id="inner_contentB" style="display:none;">
    <fieldset><form name="create_textbook" action="textbook_manage.php" method="post">
      ...
      <table width="200" border="1" class="form">
        ...
        <tr>
		<th>教材屬性：</th>
          <td><select name="attributes" id="create_attributes" >
          ...
          <th>難易度： </th>
          <td><select name="difficulty" id="create_difficulty">
        ...
        <tr>
          <th>是否公開： </th>
          <td><select name="isPublic" id="create_isPublic">
            <option>否</option>
            <option>是</option>
          </select></td>
        </tr>
        ...
        <tr>
          <th>是否提供下載：</th>
          <td><select name="select4" id="create_isDownload">
            <option value="0">否</option>
            <option value="1">是</option>
          </select></td>
        </tr>
		<tr id="set_downloadRole" >
          <th>下載使用者身分：</th>
          <td>
...
<!-- -----joyce 0328 -->
        <tr>
          <th>是否提供下載：</th>
          <td><select name="select4" id="modify_isDownload" onchange="modify_isDownload_func(this.options[this.options.selectedIndex].value)">
             <option value="0" selected >否</option>
             <option value="1">是</option>
          </select></td>
         </tr>
		<tr id="modify_downloadRole"  style="display:none;">
          <th>下載使用者身分：</th>
          <td>
		...
		<tr id="modify_license"  style="display:none;">
          <th>授權型態：</th>
          <td>
